...
export async function extractText(imageFile: File | Blob): Promise<string> {
  try {
    ...
    const rawCharCount = text.trim().length

    // Clean and filter the text
    const cleanedText = cleanOCRText(text)
    const cleanedCharCount = cleanedText.length
    const filterRate = Math.round((1 - cleanedCharCount / Math.max(rawCharCount, 1)) * 100)

    console.log(`[OCR] Completed in ${(duration / 1000).toFixed(1)}s`)
    console.log(`[OCR] Raw: ${rawCharCount} chars | Cleaned: ${cleanedCharCount} chars (${filterRate}% filtered)`)
